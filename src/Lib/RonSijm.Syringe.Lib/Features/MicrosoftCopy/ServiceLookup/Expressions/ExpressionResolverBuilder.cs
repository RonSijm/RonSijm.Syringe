...
internal sealed class ExpressionResolverBuilder : CallSiteVisitor<object, Expression>
{
    ...
    public ExpressionResolverBuilder(MicrosoftServiceProvider serviceProvider)
    {
        ...
        _buildTypeDelegate = (_, cs) => BuildNoCache(cs);
    }
    ...
    protected override Expression VisitIEnumerable(IEnumerableCallSite callSite, object context)
    {
        ...
        {
            Debug.Assert(!MicrosoftServiceProvider.VerifyAotCompatibility || !elementType.IsValueType, "VerifyAotCompatibility=true will throw during building the IEnumerableCallSite if elementType is a ValueType.");
    ...
    public static Expression GetCaptureDisposable(ParameterExpression scope)
    {
        ...
        {
            throw new NotSupportedException(Common.SR.@GetCaptureDisposableNotSupported);
        }
