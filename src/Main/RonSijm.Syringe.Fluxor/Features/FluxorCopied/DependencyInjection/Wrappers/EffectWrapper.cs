...
namespace RonSijm.Syringe.DependencyInjection.Wrappers;
...
internal class EffectWrapper<TAction> : IEffect
{
	...
	{
		var handler = CreateHandlerWithoutActionParameter(
			effectHostInstance,
			effectMethodInfos);

		return (action, dispatcher) => handler.Invoke(dispatcher);
	}
