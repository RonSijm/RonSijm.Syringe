...
public class ServiceCollectionServiceExtensionsTest
{
    ...
    public static TheoryData AddImplementationFactoryData
    {
        get
        {
            ...
            return new TheoryData<Action<IServiceCollection>, Type, Type, ServiceLifetime>
            {
                { collection => collection.AddTransient(serviceType, _ => new FakeService()), serviceType, objectType, ServiceLifetime.Transient },
                { collection => collection.AddTransient<IFakeService>(_ => new FakeService()), serviceType, serviceType, ServiceLifetime.Transient },
                { collection => collection.AddTransient<IFakeService, FakeService>(_ => new FakeService()), serviceType, implementationType, ServiceLifetime.Transient },

                { collection => collection.AddScoped(serviceType, _ => new FakeService()), serviceType, objectType, ServiceLifetime.Scoped },
                { collection => collection.AddScoped<IFakeService>(_ => new FakeService()), serviceType, serviceType, ServiceLifetime.Scoped },
                { collection => collection.AddScoped<IFakeService, FakeService>(_ => new FakeService()), serviceType, implementationType, ServiceLifetime.Scoped },

                { collection => collection.AddSingleton(serviceType, _ => new FakeService()), serviceType, objectType, ServiceLifetime.Singleton },
                { collection => collection.AddSingleton<IFakeService>(_ => new FakeService()), serviceType, serviceType, ServiceLifetime.Singleton },
                { collection => collection.AddSingleton<IFakeService, FakeService>(_ => new FakeService()), serviceType, implementationType, ServiceLifetime.Singleton },
            };
        }
    }
...
#pragma warning restore xUnit1026
    ...
    public static TheoryData TryAddEnumerableImplementationTypeData
    {
        get
        {
            ...
            return new TheoryData<ServiceDescriptor, Type, Type, ServiceLifetime>
            {
                ...
                { ServiceDescriptor.Transient<IFakeService, FakeService>(_ => new FakeService()), serviceType, implementationType, ServiceLifetime.Transient },

                { ServiceDescriptor.Scoped<IFakeService, FakeService>(), serviceType, implementationType, ServiceLifetime.Scoped },
                { ServiceDescriptor.Scoped<IFakeService, FakeService>(_ => new FakeService()), serviceType, implementationType, ServiceLifetime.Scoped },

                { ServiceDescriptor.Singleton<IFakeService, FakeService>(), serviceType, implementationType, ServiceLifetime.Singleton },
                { ServiceDescriptor.Singleton<IFakeService, FakeService >(_ => new FakeService()), serviceType, implementationType, ServiceLifetime.Singleton },
    ...
    public static TheoryData TryAddEnumerableInvalidImplementationTypeData
    {
        get
        {
            ...
            {
                { ServiceDescriptor.Transient<IFakeService>(_ => new FakeService()), serviceType, serviceType },
                { ServiceDescriptor.Transient(serviceType, _ => new FakeService()), serviceType, objectType },

                { ServiceDescriptor.Scoped<IFakeService>(_ => new FakeService()), serviceType, serviceType },
                { ServiceDescriptor.Scoped(serviceType, _ => new FakeService()), serviceType, objectType },

                { ServiceDescriptor.Singleton<IFakeService>(_ => new FakeService()), serviceType, serviceType },
                { ServiceDescriptor.Singleton(serviceType, _ => new FakeService()), serviceType, objectType },
            };
        }
    }
