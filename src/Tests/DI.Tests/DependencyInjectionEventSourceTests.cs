...
public class DependencyInjectionEventSourceTests : IDisposable
{
    ...
    public void EmitsServiceProviderBuilt()
    {
        ...
        using var provider = new SyringeServiceProvider(serviceCollection.BuildServiceProvider());

        var serviceProviderBuiltEvent = _listener.EventData.Single(e => e.EventName == "ServiceProviderBuilt");
        ...
        var serviceProviderDescriptorsEvent = _listener.EventData.Single(e => e.EventName == "ServiceProviderDescriptors");
        Assert.Equal(
            string.Join(Environment.NewLine,
                "{",
                "  \"descriptors\": [",
