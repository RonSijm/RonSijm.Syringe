...
public class ServiceCollectionKeyedServiceExtensionsTest
{
    ...
    public static TheoryData AddImplementationFactoryData
    {
        get
        {
            ...
            return new TheoryData<Action<IServiceCollection>, Type, object, Type, ServiceLifetime>
            {
                { collection => collection.AddKeyedTransient(serviceType, "some-key-1", (_,_) => new FakeService()), serviceType, "some-key-1", objectType, ServiceLifetime.Transient },
                { collection => collection.AddKeyedTransient<IFakeService>("some-key-2", (_,_) => new FakeService()), serviceType, "some-key-2", serviceType, ServiceLifetime.Transient },
                { collection => collection.AddKeyedTransient<IFakeService, FakeService>("some-key-3", (_,_) => new FakeService()), serviceType, "some-key-3", implementationType, ServiceLifetime.Transient },

                { collection => collection.AddKeyedScoped(serviceType, "some-key-4", (_,_) => new FakeService()), serviceType, "some-key-4", objectType, ServiceLifetime.Scoped },
                { collection => collection.AddKeyedScoped<IFakeService>("some-key-5", (_,_) => new FakeService()), serviceType, "some-key-5", serviceType, ServiceLifetime.Scoped },
                { collection => collection.AddKeyedScoped<IFakeService, FakeService>("some-key-6", (_,_) => new FakeService()), serviceType, "some-key-6", implementationType, ServiceLifetime.Scoped },

                { collection => collection.AddKeyedSingleton(serviceType, "some-key-7", (_,_) => new FakeService()), serviceType, "some-key-7", objectType, ServiceLifetime.Singleton },
                { collection => collection.AddKeyedSingleton<IFakeService>("some-key-8", (_,_) => new FakeService()), serviceType, "some-key-8", serviceType, ServiceLifetime.Singleton },
                { collection => collection.AddKeyedSingleton<IFakeService, FakeService>("some-key-9", (_,_) => new FakeService()), serviceType, "some-key-9", implementationType, ServiceLifetime.Singleton },
            };
        }
    }
        ...
        ServiceLifetime lifeCycle)
    {
        ...
        Assert.Equal(implementationType, descriptor.InvokeGetImplementationType());
        Assert.Equal(lifeCycle, descriptor.Lifetime);
    }
...
#pragma warning restore xUnit1026
    ...
    public static TheoryData TryAddEnumerableImplementationTypeData
    {
        get
        {
            ...
            return new TheoryData<Func<ServiceDescriptor>, Type, object, Type, ServiceLifetime>
            {
                ...
                { () => ServiceDescriptor.KeyedTransient<IFakeService, FakeService>("service2", (_,_) => new FakeService()), serviceType, "service2", implementationType, ServiceLifetime.Transient },
                ...
                { () => ServiceDescriptor.KeyedScoped<IFakeService, FakeService>("service4", (_,_) => new FakeService()), serviceType, "service4", implementationType, ServiceLifetime.Scoped },
                ...
                { () => ServiceDescriptor.KeyedSingleton<IFakeService, FakeService>("service6", (_,_) => new FakeService()), serviceType, "service6", implementationType, ServiceLifetime.Singleton },
        ...
        ServiceLifetime expectedLifetime)
    {
        ...
        Assert.Equal(expectedImplementationType, d.InvokeGetImplementationType());
    ...
    public static TheoryData TryAddEnumerableInvalidImplementationTypeData
    {
        get
        {
            ...
            {
                { ServiceDescriptor.KeyedTransient<IFakeService>(key, (_,_) => new FakeService()), serviceType, serviceType },
                { ServiceDescriptor.KeyedTransient(serviceType, key, (_,_) => new FakeService()), serviceType, objectType },

                { ServiceDescriptor.KeyedScoped<IFakeService>(key, (_,_) => new FakeService()), serviceType, serviceType },
                { ServiceDescriptor.KeyedScoped(serviceType, key, (_,_) => new FakeService()), serviceType, objectType },

                { ServiceDescriptor.KeyedSingleton<IFakeService>(key, (_,_) => new FakeService()), serviceType, serviceType },
                { ServiceDescriptor.KeyedSingleton(serviceType, key, (_,_) => new FakeService()), serviceType, objectType },
            };
        }
    }
