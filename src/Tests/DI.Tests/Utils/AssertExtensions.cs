...
public static class AssertExtensions
{
        ...
        where TNetFxExceptionType : Exception
    {
        if (IsNetFramework)
        {
            ...
            {
                var exception = Assert.Throws(typeof(TNetFxExceptionType), action);
    ...
    {
        var oce = Assert.ThrowsAny<OperationCanceledException>(testCode);
    ...
    {
        var oce = await Assert.ThrowsAnyAsync<OperationCanceledException>(testCode);
        if (cancellationToken.CanBeCanceled)
        {
            Assert.Equal(cancellationToken, oce.CancellationToken);
        }
    ...
    {
        var exception = AssertThrows<E, T>(span, action);
        Assert.Equal(expectedParamName, exception.ParamName);
        return exception;
    }
